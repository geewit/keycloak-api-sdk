...
import io.geewit.keycloak.client.api.RealmResource;
import io.geewit.keycloak.client.api.RealmsResource;
import io.geewit.keycloak.client.api.ServerInfoResource;
import io.geewit.keycloak.client.filter.BearerAuthFilter;
import io.geewit.keycloak.client.okhttp.OkHttpClients;
import io.geewit.keycloak.client.resteasy.engine.OkHttpClientEngine;
import io.geewit.keycloak.client.token.TokenManager;
...
public class Keycloak {
    ...
    Keycloak(String serverUrl, String realm, String username, String password, String clientId, String clientSecret, String grantType, ResteasyClient resteasyClient, String authtoken) {
        ...
        client = resteasyClient != null ? resteasyClient : resteasyClient(serverUrl);
        authToken = authtoken;
    ...
    private ResteasyClient resteasyClient(String serverUrl) {
        return new ResteasyClientBuilder().httpEngine(new OkHttpClientEngine(OkHttpClients.buildClient(serverUrl))).connectionPoolSize(10).build();
    }
    ...
    public <T> T proxy(Class<T> proxyClass, URI absoluteURI) {
        return client.target(absoluteURI).register(authFilter()).proxy(proxyClass);
    }
